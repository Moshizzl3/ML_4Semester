...
trainingFiles = 'week_40/digits/two-digitsData/train'  # change according to your setup
...
test_set = test_datagen.flow_from_directory(
    'week_40/digits/two-digitsData/test' ,  # path to folder with test images
    # size of output image, f.x. 28 x 28 pixel
    target_size=(targetSize, targetSize),
    batch_size=32,  # how many images to load at a time
    class_mode=classMode,  # use 'categorical' for >2 class, 'binary' for two-class problems
...
model.add(Conv2D(16, kernel_size=3, activation="relu",input_shape=[targetSize, targetSize, 1]))
model.add(MaxPool2D(pool_size=2, strides=2))
...
model.fit(x=training_set, epochs=10, validation_data=test_set)
...
if result[0][0] < 0.5:
    print("1")  # depending on the value of training_set.class_indices
else:
    print("2")

model.save("./model_1or2t.h5")
